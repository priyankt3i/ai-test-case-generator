...
<style>
    html, body {
        font-size: 0.85rem !important; /* Adjust this value as needed */
    }

    /* Target the specific test ID for tab buttons, use !important for specificity */
    button[data-testid="stTab"] {
        font-size: 4rem !important; /* Adjust font size */
        padding: 0rem 0rem !important; /* Adjust padding */
    }
    ...
    /* --- NEW RULE for inline code elements --- */
    /* Target <code> tags (typically generated by Markdown backticks) */
    /* Avoid targeting code within <pre> blocks if necessary, */
    /* but often styling `code` directly is sufficient. */
    code {
        color: #00008B !important; /* Dark Blue color */
        font-size: 1.05em !important; /* Slightly larger than surrounding text */
        /* background-color: #EEEEEE !important; Optional: Add a subtle background */
    }
        ...
        with tab1:
            ...
            if st.button("Identify Applications", key="identify_btn"):
                ...
                if not creds_ok:
                    ...
                    if llm:
                        # *** MODIFIED CALL: Added provider_name ***
                        identified = llm_integration.identify_applications(
                            st.session_state.extracted_text,
                            llm,
