...
try:
    ...
    st.error("ERROR: `streamlit-clipboard` library not found. Please install it (`pip install streamlit-clipboard`).")
    st.stop()
...
# --- Sidebar ---
# (Sidebar code remains the same - omitted for brevity)
with st.sidebar:
...
# (File processing logic remains the same - omitted for brevity)
if current_file:
    ...
    if st.session_state.extracted_text:
        ...
        # === Generate Tab ===
        # (Generate Tab logic remains the same - omitted for brevity)
        with tab1:
        ...
        # === Refactor Tab ===
        # (Refactor Tab logic remains the same - omitted for brevity)
        with tab2:
        ...
        with tab_log:
            ...
            with col2:
                ...
                # This function renders a button and handles the copy action + success message
                copy_component("Copy Log", content=log_string_to_copy, disabled=copy_button_disabled)
                # We no longer need the manual st.button, JS, st.components.v1.html, or st.toast for copy
        ...
        # (Export button logic remains the same - omitted for brevity)
        if st.session_state.generated_test_cases:
            st.markdown("---")
